...
## Temporal high availability features

| **Term**                    | **Description**                                                                                                                                             |
|-----------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Same-region replication** | Duplicates Workflows and metadata between isolation domains co-located within the same region as the primary Namespace.                                      |
| **Multi-region replication** | Duplicates Workflows and metadata from a primary Namespace to a replica located in another region.                                                            |
...
Each Temporal Cloud replicated Namespace has a specific endpoint that ensures requests go to the correct unit.
...
- A replica refers to a backup node that mirrors data or service state from an active node.
  ...
  In Temporal, the replica node is a standby Namespace that takes over from the active Namespace during a failover.
...
- Failover is the process of automatically switching to a backup system or component when the primary system fails.
  This ensures that service continuity is maintained with minimal disruption, as the system automatically shifts operations to a redundant or standby resource.
  In high availability systems, failover mechanisms detect failures and activate standby nodes, ensuring no downtime or loss of data.
  ...
  Temporal’s automated failover feature minimizes downtime by handling the failover process without manual intervention, making the system highly available and resilient.
...
If it finds unhealthy conditions, it will automatically failover the active Namespaceto a replicathat isn’t affected by the incident.
...
Workflow updates in the active region, along with associated history events, are transmitted to the replicawith a short delay.
This delay is called the replication lag.
Temporal Cloud strives to maintain a <ToolTipTerm term="P95" tooltip="P95 refers to the 95th percentile of a data set. It identifies values that are greater than or equal to 95% of that set." /> replication delay of less than 1 minute.
...
|---------------------------------|--------------------------------------|-----------------------------------|-----------------------------------|
| How many active nodes?      | One primary Namespace, one or more replicas | All nodes are active              | One primary Namespace, one or more replicas |
...
### failover
Failover is the process of automatically switching from a failed system or component to a backup or standby system.
...
Temporal typically does not rely on synchronous replication across regions due to performance considerations.
