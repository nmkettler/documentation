...
Keeping services available is crucial in distributed applications, especially during failures and other incidents.
...
When you enable high availability replication features, you allow Temporal to copy Namespace metadata and Workflow Executions to a <ToolTipTerm term="replica" tooltip="A replica is a synchronized copy of a Namespace. It ensures redundancy, fault tolerance, and minimal downtime during an incident or outage. A replica can take over operations if the primary Namespace becomes unavailable. It's synchronized up to the point of replication lag, where no further data is available." />.
...
The failover process automatically switches to a backup system or component when the primary system fails.
It ensures that even if parts of the Temporal Service fail or become unreachable, your application continues to function.
Your application experiences minimal disruption, as Temporal automatically shifts operations to a standby system.

## Replication

Replication is the process of copying and synchronizing data or services across multiple nodes or systems.
...
Each Namespace is replicated across isolation domains (data centers) or separate regions.
...
All parts of the system will eventually synchronize to a consistent view of the Namespace metadata, even if the primary Namespace and its replica temporarily lose communication.
...
Temporal's same-region Namespaces enable replication across isolation domains within a single region.
